...
1. **On GitHub:**
   - Navigate to https://github.com/dmartingalvez/bioinfo-project-template
...
3. **Create machine-specific configuration:**
   ```bash
   # Example config/config_local.yaml
   echo "base_dir: \"/path/to/your/project/data\"" > config/config_local.yaml
   echo "repo_dir: \"$PWD\"" >> config/config_local.yaml
   ```
...
paths:
  raw_data:       "1_DATA/RAW"
  processed_data: "1_DATA/PROCESSED"
...
## 6.3 `load_config.sh` – Merging Both Configurations
...
This structure exists **outside the repository**, typically on NAS or local storage.

```
my-analysis/
│
├── 1_DATA/
│   ├── RAW/                  # Original input files (read-only)
│   │   ├── sequences/        # FASTQ, BAM, VCF, or other input formats
│   │   └── metadata/
│   │
...
│   │
│   └── PROCESSED/            # Stable derived data (if needed)
│       └── cleaned/
│
├── 2_ANALYSES/
...
│       ├── 02_preprocessing/ # Trimming, filtering, etc.
│       ├── 03_main_analysis/ # Primary analysis step
...
* **DATA (`1_DATA/`)**: Contains inputs, never modified by pipeline.
...
This script will:
- Create base folders (`1_DATA`, `2_ANALYSES`, `3_FIGURES`, etc.)
